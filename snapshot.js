...
const CONFIG = {
  ...
  FY_START: '2026-01-01',
  ranges: {
    revenue:      ['5000', '5999'],     // Income — Total revenue
    costOfSales:  ['6000', '6099'],     // COGS — Total Cost of Sales
    salaries:     ['8010', '8109'],     // Salaries block (separated for EBITDA)
    // Operating expenses EXCLUDING salaries, depreciation and financial costs:
    opexA:        ['7000', '8009'],     // selling/distribution/warehousing
    opexB:        ['8110', '9198'],     // insurance → taxes & social security
    depAmort:     ['9210', '9299'],     // Depreciation
    finCosts:     ['9510', '9598'],     // Other Financial Costs (interest etc.)
    // Cash & bank
    bankCash:     ['2460', '2990'],
...
async function build() {
  ...
  const revA = -A('revenue'),  revB = -B('revenue');          // flip credit sign
  const cosA =  A('costOfSales'), cosB = B('costOfSales');
  ...
  actuals.forEach(x=>{ const a=String(x.accountNo);
    ...
    if(inRange(a,CONFIG.ranges.costOfSales)||inRange(a,CONFIG.ranges.salaries)||
  ...
  const isPL=a=>inRange(a,CONFIG.ranges.revenue)||inRange(a,CONFIG.ranges.costOfSales)||
