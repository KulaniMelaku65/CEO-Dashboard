...
// Pass YYYY-MM-DD as a command-line arg to backfill a historical date.
// e.g.  node snapshot.js 2026-02-28
const _dateArg = process.argv.slice(2).find(a => /^\d{4}-\d{2}-\d{2}$/.test(a));
const TARGET_DATE   = _dateArg || new Date().toISOString().slice(0, 10);
const IS_HISTORICAL = !!_dateArg;
...
async function build() {
  const fy = IS_HISTORICAL
    ? `?$filter=postingDate ge ${CONFIG.FY_START} and postingDate le ${TARGET_DATE}`
  ...
  const lm=new Date(TARGET_DATE+'T12:00:00').getMonth(); const labels=mN.slice(0,lm+1);
  ...
  return { asOf: new Date(TARGET_DATE+'T12:00:00').toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}),
...
(async () => {
  try {
    ...
    if (IS_HISTORICAL) console.log(`  Historical run — data as of: ${TARGET_DATE}`);
    ...
    // Save archive — uses TARGET_DATE so historical runs save to the correct date
    const archiveDir = path.join(__dirname, 'archive');
    if (!fs.existsSync(archiveDir)) fs.mkdirSync(archiveDir);
    const archiveTarget = path.join(archiveDir, `${TARGET_DATE}.json`);
